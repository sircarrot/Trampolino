...
public class DrawLineP2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        foreach (Touch touch in Input.touches)
        {
            ...
            {
                if (Camera.main.ScreenToWorldPoint(Input.mousePosition).y > (stageDimensions.y * 1) - 1.2 && Camera.main.ScreenToWorldPoint(Input.mousePosition).y < (stageDimensions.y * 1) - 3.2)
                {
                    if (nodrawn < allowedmax)
                    {
                        target = Camera.main.ScreenToWorldPoint(Input.mousePosition);
